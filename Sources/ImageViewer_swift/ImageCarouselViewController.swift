...
public class ImageCarouselViewController: UIPageViewController, ImageViewerTransitionViewControllerConvertible {
    ...
    var initialIndex = 0
    var theme: ImageViewerTheme {
        switch themeOption {
        case .automatic:
            if self.traitCollection.userInterfaceStyle == .dark {
                ImageViewerTheme.dark
            } else {
                ImageViewerTheme.light
            }
    ...
    func themeOptionOrUserInterfaceStyleDidUpdate() {
        navItem.leftBarButtonItem?.tintColor = theme.tintColor
        backgroundView?.backgroundColor = theme.color
    }
    
    var themeOption: ImageViewerThemeOption = .automatic {
        didSet {
            themeOptionOrUserInterfaceStyleDidUpdate()
        }
    }
